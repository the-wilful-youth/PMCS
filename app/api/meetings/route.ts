...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ meetings: dbData.meetings });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { title, date, time, attendees = [], agenda = '', discussionNotes = '', decisions = [], actionItems = [] } = body;
    ...
    db.logActivity(user.id || 'u-unknown', user.name || user.username, 'meeting_recorded', `Recorded meeting ${id}: ${newMeeting.title}`, 'meeting', id);
